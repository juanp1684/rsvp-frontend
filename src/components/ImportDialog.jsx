  ...
  return new Promise((resolve) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (rows.length === 0) {
          resolve([{ row: 0, field: '', message: 'El archivo está vacío.' }])
          return
        }
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          if (rowCount !== headerCount) {
            errs.push({
              row: i + 1,
              field: '',
              message: `La fila tiene ${rowCount} columna${rowCount !== 1 ? 's' : ''} pero el encabezado tiene ${headerCount}.`,
            })
          }
        }
        resolve(errs)
      } catch {
        resolve([{ row: 0, field: '', message: 'No se pudo leer el archivo. Asegúrate de que sea un CSV o Excel válido.' }])
      }
    }
      ...
      <DialogContent className="max-w-sm">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b">
              <th className="text-left py-1.5 font-medium">Columna</th>
          ...
          <tbody>
            {COLUMNS.map(({ col, required, hint }) => (
              <tr key={col} className="border-b last:border-0">
                ...
                <td className="py-1.5">
                  {required
                    ? <span className="text-green-600 font-medium">Requerido</span>
            ...
            <ul className="divide-y max-h-48 overflow-y-auto">
              {errors.map((err, i) => (
                <li key={i} className="px-3 py-2 text-xs flex gap-3">
                  {err.row > 0 && <span className="text-muted-foreground shrink-0">Fila {err.row}</span>}
        ...
        <DialogFooter className="flex-col gap-2 sm:flex-col">
          ...
          <Button className="w-full" disabled={loading} onClick={() => fileInputRef.current?.click()}>
            ...
            {loading ? 'Importando…' : errors.length > 0 ? 'Seleccionar otro archivo…' : 'Seleccionar archivo…'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
